"""
run_parallel_L48_fine.py — L=48 fine 스캔 (동시 6개)
============================================================
Windows CMD에서:
    python run_parallel_L48_fine.py

같은 폴더에 kagome_fss_v3.py가 있어야 합니다.

목적: L=48, L=128, L=256을 공정하게 비교하기 위해
      세 격자 모두 같은 fine grid(T=1.13~1.28, dT=0.005)로 재마이닝.
      (L=128 fine에서 발견된 sqrtMI 최소점 T=1.195와,
       L=48/256의 broad 최소점 T=1.21~1.23을 모두 포함하는 범위)

시드: 기존 L=48(및 L=128) 30개 분석에 쓰인 시드(6~30, 111,222,333,444,555)와 동일하게 맞춤
      → L=48, L=128, L=256 세 격자가 정확히 같은 30개 시드 집합으로 비교 가능
============================================================
"""
import subprocess
import os
